...
bool sessionless::generateKeys(Keys &keys)
{
    ...
    #ifdef ESP32
    // Use ESP32 hardware RNG for cryptographically secure randomness
    for (int i = 0; i < SHA256_SIZE_BYTES; i++)
    {
        seed[i] = esp_random() & 0xFF;
        keys.privateKey[i] = esp_random() & 0xFF;
    }
    #else
    // Fallback for non-ESP32 platforms (NOT cryptographically secure!)
    srand(time(NULL));
